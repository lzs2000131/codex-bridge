...
- **会话延续** —— `previous_response_id` 跨供应商可用（内存 LRU，或用 Redis 实现持久化/扩展）
- **内置 `web_fetch` 工具** —— 在 URL 密集场景下绕过沙箱限制
- **工具调用熔断器** —— 软警告 + 硬剥离，防止工具调用死循环
- **单文件零依赖** —— 仅一个 `proxy.mjs`（约 2000 行），无需 `npm install`（Redis 后端为可选项）
- **Docker 就绪** —— `Dockerfile` + `docker-compose.yml`（bridge + Redis），非 root、健康检查、优雅退出
...
直接 `node proxy.mjs` 时,会话历史(`previous_response_id`)存在进程内的 LRU Map ——
本地单实例够用,但重启即丢、无法多实例共享。需要持久化、可横向扩展的部署时,用自带的
compose 栈(bridge + Redis 一起启)。

```bash
cp env.example .env          # 填入供应商密钥与代理密钥
docker compose up -d         # 启动 bridge(:4000) + redis
docker compose logs -f bridge
```

bridge 通过 `REDIS_URL` 自动识别 Redis(compose 注入 `redis://redis:6379`)。启动时会打印当前后端:

```
[codex-bridge] Store   : redis (redis://redis:6379, TTL=3600s)
```

**横向扩展** —— 由于会话状态都在 Redis,可在负载均衡后跑多个副本,任意副本都能续接任意会话:

```bash
docker compose up -d --scale bridge=3   # 前面自行挂一个 LB
```

**独立镜像**(自带 Redis,或省略 `REDIS_URL` 走内存):
...
**发布** —— `scripts/docker-publish.sh` 构建并推送 version 与 `latest` 两个 tag(版本号取自 `package.json`):

```bash
REGISTRY=ghcr.io/you IMAGE=codex-bridge ./scripts/docker-publish.sh
PLATFORMS=linux/amd64,linux/arm64 REGISTRY=ghcr.io/you ./scripts/docker-publish.sh  # 多架构
./scripts/docker-publish.sh --dry-run                                               # 仅预览命令
```

镜像以非 root 用户运行,带 `/health` 健康检查,代理处理 `SIGTERM`/`SIGINT` 优雅退出(排空连接、关闭 Redis)。

> 不用 Docker 也想用 Redis:`npm install redis` 后设置 `REDIS_URL` 再 `npm start`。
> 不设 `REDIS_URL` 时 `redis` 包根本不会被加载,保持零依赖的内存路径。
...
- ✅ **Redis 共享存储** —— 持久化、跨副本的会话延续(`REDIS_URL`)
- ✅ **横向扩展** —— 无状态副本挂在负载均衡后
- ✅ **容器化** —— `Dockerfile` + `docker-compose.yml`,非 root 用户、`/health` 健康检查
- ✅ **优雅退出** —— `SIGTERM`/`SIGINT` 排空在途请求并关闭 Redis

尚未实现(欢迎贡献):

- ⬜ **自动化测试** —— `scripts/smoke.sh` 之外的单元/集成测试
- ⬜ **CI/CD** —— GitHub Actions 跑 lint/test/build/publish
- ⬜ **指标** —— Prometheus `/metrics`(请求数、延迟、上游错误、存储命中率)
- ⬜ **限流** —— 按密钥的请求/并发上限,保护上游
- ⬜ **密钥轮换** —— 不重启热加载 `PROXY_KEYS`
- ⬜ **结构化日志** —— JSON 日志 + request-id 透传以便链路追踪
...
- 可选:Redis 6+ 及 `redis` npm 包(用于共享存储后端)
